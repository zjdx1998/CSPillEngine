...
void PB_TinyMath(py::module &m) {
  py::class_<Vec2D>(m, "Vec2D")
        .def(py::init<float, float, float>(), py::arg("x") = 0, py::arg("y") = 0, py::arg("w") = 0)
        .def("__getitem__", [](const Vec2D &v, int index) { return v[index]; })
        .def("__setitem__", [](Vec2D &v, int index, float value) { v[index] = value; })
        .def(py::self *= float())
        .def(py::self /= float())
        .def(py::self += py::self)
        .def(py::self -= py::self)
        ...
        .def("__str__", [](const Vec2D &v) {
            return "Vec2D(x: " + std::to_string(v.x) + ", y: " + std::to_string(v.y) + ", w: " + std::to_string(v.w) + ")";
        });
}
