 ...
 *
 * The ResourceManager is a singleton class. It can load and record different kinds
 * of resources such as audio, image, font, and Scene(i.e tilemap) for the game.
 */
  ...
  public:
    ...
    /**
     * GetInstance will return the reference to ResourceManager object if it is created.
    ...
    /**
     * LoadResources takes a folder path as an argument and load all resources with certain
     * formats to ResourceManager. The file format that supports are audio ("wav"), fonts("ttf"),
     * image("bmp", "jpg", "png", "tif"), and Scene("scene").
     * If the folder_path is not found, it will print an error of "folder_path is not found".
     * @param folder_path a string, the ResourceManager will search and load resources
     */
    ...
    /**
     * Search for the audio resource by audio_name. If audio_name is not loaded to the
     * ResourceManager yet, it will log "Not found audio_name" and return a nullptr.
     * Otherwise return a pointer to Mix_Chunk, which is an audio resource that can be
     * played.
     * @param audio_name a string, the name of the audio
     * @return a pointer to Mix_Chunk, if audio_name not found, nullptr is returned instead
     */
    Mix_Chunk *LoadAudio(const std::string &audio_name);

    /**
     * Search for the font resource by font_name. If font_name is not loaded to the
     * ResourceManager yet, it will log "Not found font_name" and return a nullptr.
     * Otherwise return a pointer to TTF_Font, which is a font resource that can be
     * displayed.
     * @param font_name a string, the name of the font
     * @return a pointer to TTF_Font, if font_name not found, nullptr is returned instead
     */
    TTF_Font *LoadFont(const std::string &font_name);

    /**
     * Search for the image resource by image_name. If image_name is not loaded to the
     * ResourceManager yet, it will log "Not found image_name" and return a nullptr.
     * Otherwise return a pointer to SDL_Texture, which is an image resource that can be
     * displayed.
     * @param image_name a string, the name of the image
     * @return a pointer to SDL_Texture, if image_name not found, nullptr is returned instead
     */
    SDL_Texture *LoadImage(const std::string &image_name);

    /**
     * Search for the tile map resource by scene_name. If scene_name is not loaded to the
     * ResourceManager yet, it will log "Not found scene_name" and return a nullptr.
     * Otherwise return a pointer to Scene, which is a tile map resource that can be
     * displayed.
     * @param scene_name a string, the name of the scene
     * @return a pointer to Scene, if scene_name not found, nullptr is returned instead
     */
     ...
     * in the ResourceManager, it will return an empty string. Otherwise, it returns
     * the string of the active scene.
    ...
    /**
     * Get the file path to the active scene. If the active scene name is not found
     * in the ResourceManager, it will return a nullptr. Otherwise, it returns
     * the string of the active scene.
     * @return a string of the file path to the active scene, if the name is not found,
     *          return an empty string instead
     */
     ...
     * in the ResourceManager, it will return false. Otherwise, it sets active scene to
     * scene_name and returns true.
     * @param scene_name a string of scene name that is going to be set as active scene
