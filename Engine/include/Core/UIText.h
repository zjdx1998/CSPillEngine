...
class UIText: public GameObject {
  public:
    static std::unique_ptr<GameObject> Create(std::string_view font_name_, std::string_view content, const CSPill::Math::Vec2D &pos, const CSPill::Math::Vec2D &size, int font_size);
  ...
  private:
    UIText(std::string_view font_name, std::string_view content, const CSPill::Math::Vec2D &pos, const CSPill::Math::Vec2D &size, int font_size);
