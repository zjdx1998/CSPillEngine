...
std::unique_ptr<GameObject> UIText::Create(std::string_view font_name, std::string_view content, const CSPill::Math::Vec2D &pos, const CSPill::Math::Vec2D &size, int font_size) {
  auto* ui_text = new UIText(font_name, content, pos, size, font_size);
  return std::unique_ptr<GameObject>(ui_text);
}

UIText::UIText(std::string_view font_name, std::string_view content, const CSPill::Math::Vec2D &pos, const CSPill::Math::Vec2D &size, int font_size)
    : GameObject(), font_name_(font_name), content_(content), pos_(pos), size_(size), font_size_(font_size), text_surface_(nullptr), text_texture_(nullptr) {}
...
void UIText::SetFontSize(int font_size) {
    font_size_ = font_size;
}

const CSPill::Math::Vec2D &UIText::GetSize() const {
    return size_;
}

void UIText::SetSize(const CSPill::Math::Vec2D &size) {
    size_ = size;
}

const CSPill::Math::Vec2D &UIText::GetPos() const {
    return pos_;
}

void UIText::SetPos(const CSPill::Math::Vec2D &pos) {
    pos_ = pos;
}
