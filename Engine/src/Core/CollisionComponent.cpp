...
const std::function<void(GameObject *)> &CollisionComponent::GetCallback() const {
  return callback_;
}
void CollisionComponent::SetCallback(const std::function<void(GameObject *)> &callback) {
  callback_ = callback;
}
