...
void CameraComponent::Update(GameObject *object, float dt) {
  auto transform_component = dynamic_cast<TransformComponent *>(object->GetComponent(
  ...
  if (bind_object_) {
    if (auto transform_bind_object = bind_object_->GetComponent(
      ...
      transform_component->position() =
          dynamic_cast<TransformComponent *>(transform_bind_object)->position() + offset_;
      velocity = {dynamic_cast<TransformComponent *>(transform_bind_object)->velocity().x,
                  dynamic_cast<TransformComponent *>(transform_bind_object)->velocity().y};
    }
  }
  ...
  x = std::fmax(0.f,
                std::fmin(x + dt * velocity.x * transit_, transform_component->position().x + view_range_.x / 2));
  y = std::fmax(0.f,
                std::fmin(y + dt * velocity.y * transit_, transform_component->position().y + view_range_.y / 2));
...
const Math::Vec2D &CameraComponent::GetViewRange() const {
  return view_range_;
}
void CameraComponent::SetViewRange(const Math::Vec2D &view_range) {
  view_range_ = view_range;
}
float CameraComponent::GetTransit() const {
  return transit_;
}
void CameraComponent::SetTransit(float transit) {
  transit_ = transit;
}
